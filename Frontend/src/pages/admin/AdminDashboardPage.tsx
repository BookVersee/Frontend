...
import { Order, Transaction, User, OrderStatus, ReturnStatus } from "../../types";
...
type AdminPageTab = "overview" | "orders" | "returns" | "transactions" | "users";
...
export const AdminDashboardPage: React.FC = () => {
  ...
  const handleReturnDecision = async (orderId: number, status: ReturnStatus) => {
  ...
  const navItems = [
    ...
    { key: "returns" as const, label: "Hoàn hàng & Đổi trả", icon: <RefreshCw size={16} /> },
    { key: "transactions" as const, label: "Lịch sử giao dịch", icon: <CreditCard size={16} /> },
    { key: "users" as const, label: "Người dùng & Cửa hàng", icon: <Users size={16} /> },
  ];
